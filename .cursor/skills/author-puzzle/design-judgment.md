...
- **Lenses** default to sequential reclassification. Use `quiz` or `assignment` when the pedagogy needs them, not by habit. Trivia often leans `quiz` + `preSolve: true`; that is a lean for that category, not a rule. A lens that a player can answer by picking one cluster's color is not cross-cutting unless that concentration is the point. Bound the prompt; check the most plausible excluded node.

...
export async function run(page, baseURL) {
  ...
  await page.waitForSelector("#puzzle-title:not(:empty)");
  assert.equal(await page.evaluate(() => mode), "sets", "&mode=sets should override a stored 'graph' preference");
  ...
  await page.goto(`${baseURL}/index.html`);
  await page.waitForSelector("#puzzle-title:not(:empty)");
  assert.equal(await page.evaluate(() => mode), "graph", "&mode= must not overwrite the stored preference for later, param-less visits");

  // A garbage &mode= value falls back to the normal stored/default logic.
  await page.goto(`${baseURL}/index.html?mode=bogus`);
  await page.waitForSelector("#puzzle-title:not(:empty)");
  assert.equal(await page.evaluate(() => mode), "graph", "an invalid &mode= value should fall back to the stored preference, not error");

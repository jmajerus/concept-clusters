...
describe("hosted authoring Worker", () => {
  ...
  it("answers 410 for the retired admin routes", async () => {
    // Admin moved to the LAN authoring server. These paths existed here and
    // deliberately do not any more, so a stale bookmark should say so rather
    // than look like an outage -- and the Worker must never half-serve one.
    for (const path of [
      "/admin",
      "/admin/drafts",
      "/admin/drafts/energy-flow",
      "/admin/catalogues",
      "/admin/categories",
      "/admin/link-health"
    ]) {
      ...
      expect(response.status, `${path} should be gone, not served`).toBe(410);
      expect(await response.text()).toContain("Admin has moved");
    }

    // A path this Worker never owned is still a plain 404.
    const unknown = await worker.fetch(
      new Request("http://localhost:8788/nope"),
      env,
      createExecutionContext()
    );
  ...
  it("canonicalizes leftover link fields when get_puzzle_draft loads a stored draft", async () => {
    ...
    // The banner this flag drives is rendered by the admin page, which now
    // lives only on the LAN authoring server; tests/draft-review-page.mjs and
    // tests/local-draft-review.mjs assert it there.
  });

...
review happens on `/admin/drafts` *before* a git freeze (same pause
as local stdio MCP). **Publish** on that page writes the shared D1 document.
**Cue** that snapshot when it should join the next freeze. **Freeze** on
LAN `/admin` writes git; the hosted Worker only shows the plan. The MCP
submit tool remains for catalogue extras and for clients that ask for a
GitHub pull request. Hosted authoring has no git checkout and does not
write the base branch; this repo does not auto-deploy the player-facing
Worker on push. Merging a GitHub pull request stays a separate human
action, so opening one does not update the bundled player by itself.

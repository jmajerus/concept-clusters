...
review happens on `/admin/drafts` (same pause as local stdio MCP).
**Publish** on that page writes the shared D1 document. The MCP submit tool
remains for leftover GitHub pull requests when a client asks. Hosted
authoring has no git checkout and does not write the base branch; this repo
does not auto-deploy the player-facing Worker on push. Merging a GitHub
pull request stays a separate human action, so opening one does not update
the bundled player by itself.

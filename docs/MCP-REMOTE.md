...
as local stdio MCP); the usual next step is the **Open pull request**
button on that page. The MCP tool remains for catalogue extras and for
clients that are not looking at the page. Hosted authoring has no git
checkout and does not write the base branch; this repo does not auto-deploy
the player-facing Worker on push, so there is no install-to-production
button. Merging the resulting pull
request stays a separate, always-required human action in GitHub, so
opening it doesn't publish anything by itself.

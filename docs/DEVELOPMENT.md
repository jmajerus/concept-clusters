...
`/admin/categories` — see [AUTHORING.md](AUTHORING.md) and
...
`node tools/authoring-workspace.mjs` to print the resolved paths. Review logs
and other scratch files belong there, not in the MCP protocol workspace.
...
| `puzzles/` | **The authoring format.** One file per puzzle, grouped into category subdirectories, each exporting its puzzle manifest; a resource-bearing puzzle may also have id-prefixed Markdown/assets beside it. `puzzles/index.js` is the Node/CI eager registry (`PUZZLES`); `puzzles/manifest.js` (generated by `npm run puzzles:build-manifest`) is what the player loads at boot, with full modules fetched on demand. Adding a puzzle requires no game-code changes — see [AUTHORING.md](AUTHORING.md) |
...
| `content/` | Versioned canonical simplified puzzle sources plus JSON-LD interchange/context and JSON Schema contracts — see [CANONICAL-CONTENT.md](CANONICAL-CONTENT.md) and [JSON-LD.md](JSON-LD.md) |
| `.concept-clusters/` | Git-ignored local state. Remnant file drafts may still live under `drafts/`. Authoring scratch (review log, inventories, split plans, loss ledgers, proposals) lives under `authoring/` unless `AUTHORING_DATA_DIR` points elsewhere — see [Local development](DEVELOPMENT.md#local-development) |
...
| `modules/wikipediaTitles.js`, `modules/wikiLinkCheck.js`, `modules/wikiLinkCheckStore.js` | One resolver for "does this Wikipedia title exist", the per-document / corpus-wide check built on it, and its D1-backed memory (`wiki_link_checks`) — shared by the CLI, the draft page, `check_puzzle_links`, and the authoring Worker's weekly cron |
...
| `draftReviewSubmit.js` | Same-origin POST helper for Publish, Publish & Cue, Cue/Hold, Revert, withdraw, and delete from `/admin/drafts/<id>` | — |
...
| `localDevHttp.js` | Shared local HTTP bootstrap: `npm run dev`, optional Worker proxy, and `npm run admin`; owns the dev-server lease and bounded shutdown (including detached Wrangler child process groups); Freeze refreshes the GitHub production snapshot (joined with the freeze patch); Refresh from GitHub writes origin membership only | `localDevHousekeep.js`, `localDraftReview.js`, `contentInterchangeService.js`, `authoringWorkspacePaths.js`, `githubProductionManifest.js`, `tests/lib/server.mjs` |
...
```
npm run check-wiki-links            # trusts titles checked in D1 within the last week
...
an acceptable steady state for a board term. Results are remembered in
D1's `wiki_link_checks` table — the same rows the authoring Worker's weekly
cron, the draft page's link flags, and the `check_puzzle_links` MCP tool
share — so re-running only hits the network for titles nobody has checked
recently. Without D1 configured (`.env`, see `modules/localD1Config.js`) it
still runs, asking Wikipedia for everything.
...
- **Weekly link-health check** — a cron trigger on the *authoring* Worker
  (`wrangler.authoring.jsonc`, `scheduledLinkHealth` in
  `src/authoring-worker.ts`) collects every authored `wiki:` link from the
  published puzzles in D1 and resolves them with the same
  `modules/wikipediaTitles.js` the CLI uses, writing results to
  `wiki_link_checks` and issues to this dashboard's analytics. Reading the
  title set from D1 means it can never go stale: the old bundled
  `src/link-manifest.json` only changed when someone remembered to re-run
  the CLI, and went months without. Wikipedia article titles do
  occasionally get renamed or merged; this catches that drift
  automatically.

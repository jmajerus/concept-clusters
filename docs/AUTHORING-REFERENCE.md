# Authoring field and edge-case reference

This annex contains the exhaustive schema examples, field mechanics,
extended rationale, and uncommon presentation options for Concept Clusters
puzzles. Start with [Authoring a puzzle](AUTHORING.md), which keeps the
governing design tradeoffs and normal workflow together.

AI agents should retrieve `get_authoring_guidance` and
`get_authoring_schema` by phase before consulting this file. Load only the
specific section needed for an unusual field, edge case, or validation
problem; do not place this complete annex into routine authoring context.

The schema examples below show the runtime puzzle object where useful. MCP
drafts use the equivalent [simplified puzzle
format](SIMPLIFIED-PUZZLE-FORMAT.md), and `get_authoring_schema` is the
machine-readable field contract.

## Direct repository mechanics

Puzzles are plain data, one file per puzzle under `puzzles/<category>/`
(for example, `puzzles/science/energy-flow.js`), each default-exporting one
puzzle object. A puzzle with packaged resources such as a learning
introduction uses `definePuzzle(import.meta.url, { ... })` so relative files
resolve safely; ordinary puzzles need no wrapper.

For a direct repository addition, create the module in its category directory,
then import it and add it to `PUZZLES` in `puzzles/index.js`. Registration order
is not player-facing navigation order: share links and other references use the
puzzle's `id`, never its array position. Run `node validate.mjs` after editing.
It checks term and seed counts, collisions, `idealTerms`, bridge topology,
Concept Lens targets and reason keys, and the other schema rules below.

Pull requests opened through MCP intentionally omit the `puzzles/index.js`
change. Concurrent submissions would otherwise conflict on that shared file,
and GitHub does not support `merge=union` for JavaScript import lists.
`tools/ensure-puzzle-registry.mjs` runs in CI so validation sees the module and
runs again after merge through the Sync puzzle registry workflow. On those
pull requests, an on-disk module without a matching index entry is expected.

For exchanging a complete puzzle—or installing one without manually editing
its module and registry—use the [simplified
format](SIMPLIFIED-PUZZLE-FORMAT.md), which is also used by MCP drafts and
canonical `content/puzzles/*.ccpuzzle.json` files. `content:import` and
`content:export` speak JSON-LD specifically; see [JSON-LD.md](JSON-LD.md).
JSON-LD is portable interchange, not the everyday authoring format.
...
that is the canonical endpoint of the authored relationship — e.g. the
`veto` bridge connects to Roman Republic through `tribunes`, not `Senate`
or `consuls`. This is part of the board's topology, not a record of which
node a player happened to tap.

Connecting to **any** completed node in the right cluster still counts
as selecting the correct cluster — requiring one specific tap would
recreate the exact trap-word guessing game the "no trap words" rule exists
to avoid (now between cluster-mates instead of within a cluster). The board
nevertheless resolves that bridge side to the author's canonical endpoint.
If the endpoint term has not yet been placed, the line waits at a stable
cluster proxy rather than revealing the term's answer; it moves to the
endpoint when the term is placed. Tapping the endpoint directly still earns
brief recognition, but a different valid tap never creates a different
graph.
